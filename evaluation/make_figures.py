...
def fig_loop_closure():
    ...
    ax = axes[1]
    names = ["ATE unaligned", "ATE Sim(3)-aligned"]
    off_vals = [float(d["off_ate_unaligned"]), float(d["off_ate_aligned"])]
    on_vals = [float(d["on_ate_unaligned"]), float(d["on_ate_aligned"])]
    x = np.arange(len(names)); w = 0.36
    b1 = ax.bar(x - w / 2, off_vals, w, label="OFF", color="#a0aec0")
    b2 = ax.bar(x + w / 2, on_vals, w, label="ON", color="#c53030")
    for bars in (b1, b2):
        for b in bars:
    ...
    ax.set_title(f"Effect of loop closure ({int(d['n_loops_on'])} closures flagged)")
    ax.legend(fontsize=9); ax.grid(alpha=0.3, axis="y")
